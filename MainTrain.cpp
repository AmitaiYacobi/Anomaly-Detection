...
void generateTrainCSV(float a1, float b1, float a2, float b2, float a3,
                      float b3) {
...
void generateTestCSV(float a1, float b1, float a2, float b2, float a3,
                     float b3) {
...
void checkCorrelation(correlatedFeatures c, string f1, string f2, float a,
                      float b) {
  if (c.feature1 == f1) {
    if (c.feature2 != f2)
...
int main() {
  ...
  for_each(r.begin(), r.end(), [&detected](AnomalyReport ar) {
    if (ar.description == anomalies[0].description &&
        ar.timeStep == anomalies[0].timeStep)
      detected[0] = true;
    if (ar.description == anomalies[1].description &&
        ar.timeStep == anomalies[1].timeStep)
      detected[1] = true;
    if (ar.description == anomalies[2].description &&
        ar.timeStep == anomalies[2].timeStep)
      detected[2] = true;
  });
