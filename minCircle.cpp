...
Circle from3Points(Point a, Point b, Point c) {
  ...
  */
  float x = (-pSlopBC * mBC.x + mBC.y + pSlopAB * mAB.x - mAB.y) /
            (pSlopAB - pSlopBC);
...
Circle trivial(vector<Point>& P) {
  ...
  if (dist(P[2], c.center) <= c.radius) return c;
  c = from2points(P[0], P[2]);
  if (dist(P[1], c.center) <= c.radius) return c;
  c = from2points(P[1], P[2]);
  if (dist(P[0], c.center) <= c.radius) return c;
...
Circle welzl(Point** P, vector<Point> R, size_t n) {
  ...
  if (dist(p, c.center) <= c.radius) return c;
